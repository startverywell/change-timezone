!(function (c, M) {
  ...
  function X() {
    for (
      var c,
        ...
        o = new W(new Date(A, 0, 1)),
        z = [o],
        b = 1;
      b < 48;
      b++
    )
      (M = new W(new Date(A, b, 1))).offset !== o.offset &&
        ((c = (function (c, M) {
          for (var A, o; (o = 6e4 * (((M.at - c.at) / 12e4) | 0)); )
            (A = new W(new Date(c.at + o))).offset === c.offset
              ? (c = A)
              : (M = A);
          return c;
        })(o, M)),
        z.push(c),
        z.push(new W(new Date(c.at + 6e4)))),
        (o = M);
    for (b = 0; b < 4; b++)
      z.push(new W(new Date(A + b, 0, 1))),
        z.push(new W(new Date(A + b, 6, 1)));
    return z;
  }
  function B(c, M) {
  ...
  function u() {
      ...
      var A,
        ...
        a = (function (c) {
          for (var M, A, o = c.length, z = {}, b = [], a = 0; a < o; a++)
            for (M in (A = e[c[a].offset] || {}))
              A.hasOwnProperty(M) && (z[M] = !0);
    ...
    return p.sort(B), 0 < p.length ? p[0].zone.name : void 0;
  }
  function l(c) {
    ...
    (b.updateOffset = function (c, M) {
      ...
      void 0 === c._z &&
        (z &&
          C(c) &&
          !c._isUTC &&
          ((c._d = b.utc(c._a)._d), c.utc().add(z.parse(c), 'minutes')),
        (c._z = z)),
        c._z &&
          ((o = c._z.utcOffset(c)),
          Math.abs(o) < 16 && (o /= 60),
          void 0 !== c.utcOffset
            ? ((A = c._z), c.utcOffset(-o, M), (c._z = A))
            : c.zone(o, M));
    }),
    (P.tz = function (c, M) {
      if (c) {
